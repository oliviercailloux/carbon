...
\begin{example}
	...
	Set $τ(H_{a_1}) = 1 - 2/2 + 20/2 - 10 = 0$, $τ(H_{a_2}) = 2 - 2/2 + 20/2 - 5 = 5$, $τ(H'_{a_1}) = ?$.
...
Below, we wonder whether
$\forall T, M, C, \exists τ, M' \suchthat \forall H \in T, \sum_{a \in H} c(a) + τ(H_a) ≤ M' ⇔ H \in F$.
...
Trying to make $τ$ depend on the pollution only, let’s try to set $τ(H_a) = f(p(a))$.
We have that $H \in F ⇔ \sum_{a \in H} p(a) ≤ C$. 
We want: $\sum_{a \in H} p(a) ≤ C ⇔ \sum c(a) + \sum f(p(a)) ≤ M'$.
Now assume that for $H1 \in F$, $\sum c(a) = 10$ while $\sum p(a) = C$.
This requires that $10 + f(C) ≤ M'$.
Also assume that for $H2 \notin F$, $\sum c(a) = 5$ while $\sum p(a) = C + 1$.
This requires that $M' < 5 + f(C + 1)$.
We obtain $10 + f(C) ≤ M' < 5 + f(C + 1)$ or $5 + f(C) ≤ M' < f(C + 1)$.

Adding now some $H$ that are not raw-cost-f.
Thus $H \notin F$ and $\sum_{a \in H} c(a) > M$.
Say $\sum c(a) = M + 1$.
We want: $\sum c(a) + \sum f(p(a)) > M'$. 
Thus: $\sum f(p(a)) > M' - M - 1$.
Now assume $p(a) = C$. We get: $\sum f(C) > M' - M - 1$.
Joining, we get $M' - M - 1 < f(C) ≤ M' - 5$ thus $- M - 1 < 5$.
Now assume $p(a) = C + 1$. We get: $\sum f(C + 1) > M' - M - 1$.
Joining, we get $M' - M - 1 < f(C) ≤ M' - 5$ thus $- M - 1 < 5$.

As a result, my personal uncertain guess is in favor of existence.

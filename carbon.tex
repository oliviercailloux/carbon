...
%Define $C_i = \sum_{j = i}^n c_j$. Define $T_i = \sum_{j = 1}^i t_i$ (so that $T_1 = t_1$).
%It follows that $l_n^{t_n} = \sum C_i λ^{t_n - T_i}$. As $l_{n - 1}^{t_{n - 1}} = \sum C_i λ^{t_{n - 1} - T_i}$, and $λ^{t_n - T_i} λ^{t_{n - 1} - t_n} = λ^{t_{n - 1} - T_i}$, 
...
%Also, $l_j^{t_j} = l_{j - 1}^{t_{j - 1}} x + c_j$… And, $l_j^{t_j} = l_{j - 1}^{t_j} + c_j$. Thus, $l_{j - 1}^{t_j} = l_{j - 1}^{t_{j - 1}} x$. And $λ^{t_{j - 1} - t_i} λ^{t_j - t_{j - 1}} = λ^{t_j - t_i}$.

...
We call \emph{history} a finite sequence of acts, denoted generically as $H$. 
We assume that all elements of a history are different. 
...
The lab would like to choose the best history of acts to realize among a set $T \subseteq \allHistories$ which denote the set of possible histories for the current year; her preferences is described by a \emph{welfare ordering} $\succeq$ (a linear order) over $\allHistories$.
...
Let $τ: \allHistories → \R^+$ denote the tax function. (The set $\R^+$ includes zero.)  
Given a history $H$, the taxe function only depends only on the subsequence $H^{m}$ where $m = m(H_{|H|})$ is the lab member making the last operation in the history, $\tau(H) = \tau(H^{m})$. 
...
We suppose that the lab associates a utility $u(H)$ to every history (\HG{is $u$, linear, submodular, should we consider the utility of the sequence or the increment in utility.}) and decides to grant the act $a$ according to some function $f(u(a), c(a) + τ(a)) \in \set{0, 1}$. An act $a$ is granted iff $f(u(a), c(a) + τ(a)) = 1$.
...
\begin{example}
	$T = \set{B_1 = {a_1, a_2}, B_2 = {a}}$, 
	$u(a_1) = u(a_2) = 1, u(a) = 0.5$;
	all acts contain only one action;
	$p(a_1) = p(a_2) = 2T$; $p(a) = 3T$; $H = 3T$; and raw costs of all acts are equal, as a result $F = \{B_2\}$.
	$B_1 \succ B_2$.
...
Now assume that $T$ is \emph{downwards-closed}, meaning that $B \in T ⇒ \forall b \in B: B \setminus \set{b} \in T$.

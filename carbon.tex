...
Recall that $\allacts$ denotes the set of \emph{acts}, where an act is an action performed by an actor in some context. 
We call \emph{bundle} a finite set of acts, denoted generically as $B$.
...
The lab would like to choose the best bundle of acts to perform among a set $T \subseteq \allbundles$ which denote the set of possible bundles for the current year; her preferences is described by a \emph{welfare ordering} $\succeq$ (a weak order) over $\allbundles$.
However, recall that each act $a$ has a bare cost $c_a = c(a)$ (in €) and its main action $α$ has a pollution level $p(α) = p_a = p(a)$ (in CO²eq). 
The choices of the lab are restricted by a budget $M$ in € (hard constraint) and a carbon target $H$ in CO²eq (soft constraint).

A bundle $B$ is \emph{raw-cost-feasible} iff $\sum_{a\in B} c_a ≤ M$; it is \emph{CO²-feasible} iff $\sum_{a\in B} p_a ≤ H$.
Let $F \subseteq T$ denote the \emph{a priori feasible} bundles, meaning, the bundles of $T$ which are raw-cost-feasible and CO²-feasible.

The lab would like to design a tax function to ensure ()
Let $τ: \allacts → \R^+$ denote the tax function. (The set $\R^+$ includes zero.)
A cap is a value $M' ≥ M$.
A bundle $B$ is tax-feasible (for a given cap $M'$) iff $\sum_{a\in B} (c_a + τ_a) ≤ M'$.

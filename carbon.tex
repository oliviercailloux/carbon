...
\subsection{Welfare}
Given an integer $i \in \mathbb{N}$, we use $[n]$ to denote the set $\{1,\ldots, n\}$. 
...
We suppose that the lab associates a utility $u(H)$ to every history (\HG{is $u$, linear, submodular, should we consider the utility of the sequence or the increment in utility.}) such that $u(()) = 0$. 
...
We thus wonder in which conditions $\forall {\succeq}, T, M, C \suchthat F ≠ \emptyset \land \exists g$ that selects a $\succeq$-maximal element of the raw-cost-feasible histories of $T$ with $g$ WP: (P1). %$\exists τ, M', f$ such that $τ, f$ are WP and the sequence of granted acts constitute a history that is a $\succeq$-maximal element of $F$ and is tax-feasible. 
...
I will try finding a counter example.
